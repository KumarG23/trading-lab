...
def format_daily_summary(summary: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Today: {int(summary.get('proposals') or 0)} proposals · {int(summary.get('closes') or 0)} closes · {int(summary.get('wins') or 0)}W/{int(summary.get('losses') or 0)}L · {money} · {total_r:+.2f}R · DD {drawdown_r:.2f}R",
            f"Strategies: {strategies}",
            f"Open: {int(summary.get('active_positions') or 0)} · errors: {int(summary.get('errors') or 0)}",
